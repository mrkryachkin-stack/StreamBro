...
export default function RegisterPage() {
  ...
  if (success) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", background: "var(--bg-0)" }}>
        <div style={{ background: "rgba(15,15,40,0.8)", border: "1px solid rgba(201,162,39,0.15)", borderRadius: "var(--r-xl)", padding: "3rem 2.5rem", width: "100%", maxWidth: 420, textAlign: "center", backdropFilter: "blur(24px)", animation: "scaleIn 0.3s var(--ease-spring)", boxShadow: "0 32px 80px rgba(0,0,0,0.4)" }}>
          <div style={{ width: 60, height: 60, borderRadius: "50%", background: "rgba(42,157,92,0.1)", border: "1px solid rgba(42,157,92,0.2)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1.25rem", fontSize: "1.5rem" }}>✉️</div>
          <h2 style={{ fontWeight: 800, fontSize: "1.3rem", letterSpacing: "-0.025em", marginBottom: "0.6rem" }}>Проверьте почту</h2>
          <p style={{ color: "var(--text-2)", fontSize: "0.88rem", marginBottom: "0.5rem", lineHeight: 1.6 }}>
            Мы отправили письмо на <strong style={{ color: "var(--text-1)" }}>{form.email}</strong>. Нажмите ссылку в письме для подтверждения.
          </p>
          <p style={{ color: "var(--text-2)", fontSize: "0.78rem", marginBottom: "1.75rem", lineHeight: 1.5 }}>
            Без подтверждения email некоторые функции могут быть ограничены.
          </p>
          <a href="/dashboard" className="btn-gold" style={{ display: "inline-flex", fontSize: "0.9rem", padding: "0.75rem 2rem" }}>
            Перейти в кабинет
          </a>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "stretch", background: "var(--bg-0)", position: "relative", overflow: "hidden" }}>
      {/* Background layers */}
      <div style={{ position: "fixed", inset: 0, background: "radial-gradient(ellipse 70% 60% at 70% 50%, rgba(124,92,191,0.04) 0%, transparent 60%)", pointerEvents: "none" }} />
      <div style={{ position: "fixed", inset: 0, background: "radial-gradient(ellipse 50% 50% at 20% 30%, rgba(201,162,39,0.04) 0%, transparent 60%)", pointerEvents: "none" }} />
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "4rem 5%", maxWidth: "50%", position: "relative" }}>
        ...
        <div>
          <span style={{ fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--gold)", display: "block", marginBottom: "0.75rem" }}>Создать аккаунт</span>
          <h1 style={{ fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 900, letterSpacing: "-0.04em", lineHeight: 1.05, marginBottom: "1.25rem" }}>
            Начни<br />
            <span style={{ fontFamily: "Georgia, serif", fontStyle: "italic", fontWeight: 400, color: "var(--gold)" }}>стримить</span>
          </h1>
          <p style={{ color: "var(--text-2)", fontSize: "0.92rem", lineHeight: 1.65, maxWidth: 300 }}>
            Аккаунт нужен для P2P со-стрима и друзей. Скачивание и запись работают без аккаунта.
          </p>
        </div>

        <div style={{ marginTop: "auto", paddingTop: "4rem", display: "flex", gap: "0.5rem" }}>
          {["P2P со-стрим", "Друзья", "Облако"].map((tag) => (
            <div key={tag} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "0.35rem 0.75rem", borderRadius: 999, background: "rgba(201,162,39,0.06)", border: "1px solid rgba(201,162,39,0.12)", fontSize: "0.72rem", color: "var(--gold)", fontWeight: 600 }}>
              {tag}
            </div>
          ))}
        </div>
          ...
          <div style={{ background: "rgba(15,15,40,0.8)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "var(--r-xl)", padding: "2.75rem 2.25rem", backdropFilter: "blur(24px)", boxShadow: "0 32px 80px rgba(0,0,0,0.4)" }}>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: "1rem" }}>
                <label style={{ display: "block", marginBottom: "0.45rem", fontSize: "0.78rem", fontWeight: 600, color: "var(--gold)", letterSpacing: "0.07em", textTransform: "uppercase" }}>Email</label>
              ...
              <div style={{ marginBottom: "1rem" }}>
                <label style={{ display: "block", marginBottom: "0.45rem", fontSize: "0.78rem", fontWeight: 600, color: "var(--gold)", letterSpacing: "0.07em", textTransform: "uppercase" }}>Имя пользователя</label>
            ...
            <p style={{ textAlign: "center", marginTop: "1.5rem", color: "var(--text-2)", fontSize: "0.8rem" }}>
              Уже есть аккаунт?{" "}
